...
// `GraphDB` methods are reached through the `T: sovereign_db::GraphDB` bound, and
// `SurrealGraphDB` is only used by the tests below — both imports live where used
// to avoid unused-import warnings in the lib build.

// Moved from sovereign-app into sovereign-ai so both the Tauri binary and the
// native shell can seed sample data. sovereign-ai depends on sovereign-crypto
// unconditionally, so the former `#[cfg(feature = "encryption")]` gates (which
// don't exist as a feature here) are dropped — the crypto-backed PII seed is
// always available.
use std::sync::Arc;
...
mod tests {
    use super::*;
    // GraphDB trait must be in scope for its methods (list_entities, etc.) to
    // resolve on the concrete SurrealGraphDB the tests use.
    ...
    #[test]
    fn seed_profile_and_history_writes_and_is_idempotent() {
        use sovereign_core::profile::UserProfile;

        let dir = std::env::temp_dir().join(format!("seed-profile-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir); // clean slate

        // First seed: creates the demo profile + multi-day session log.
        seed_profile_and_history(&dir).unwrap();

        let profile_path = dir.join("user_profile.json");
        let log_path = dir.join("session_log.jsonl");
        assert!(profile_path.exists(), "should write user_profile.json");
        assert!(log_path.exists(), "should write session_log.jsonl");

        let profile = UserProfile::load(&dir).unwrap();
        assert_eq!(profile.display_name.as_deref(), Some("Alex"));
        assert_eq!(profile.nickname.as_deref(), Some("Ike"));

        let lines_after_first = std::fs::read_to_string(&log_path).unwrap().lines().count();
        assert!(lines_after_first > 0, "session log should have entries");

        // Second seed: existing files are left untouched (no duplicate entries).
        seed_profile_and_history(&dir).unwrap();
        let lines_after_second = std::fs::read_to_string(&log_path).unwrap().lines().count();
        assert_eq!(
            lines_after_first, lines_after_second,
            "re-seeding must not append duplicate session-log entries"
        );
